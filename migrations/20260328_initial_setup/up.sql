...
('DAILY','FIRST_DAY_WEEK','LAST_DAY_WEEK','FIRST_DAY_MONTH','LAST_DAY_MONTH')), -- 12.1d, 11.1
    funding_scheme          TEXT NOT NULL CHECK (funding_scheme IN ('ZERO_LIABILITY','FIXED_FUND')),       -- 12.1e, 8.1
    status                  TEXT NOT NULL CHECK (status IN ('DRAFT','ACTIVE','SUSPENDED','TERMINATED')),   -- 14 status filter
    created_at              TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS nj_level (
    ...
    trigger_type            TEXT NOT NULL CHECK (trigger_type IN ('FIXED_ODDS','MYSTERY_BOUNDARY','TIMER_COUNTDOWN','MULTIDROP')),
...
CREATE TABLE IF NOT EXISTS nj_level_trigger (
    ...
    type                    TEXT NOT NULL CHECK (type IN ('FIXED_ODDS','MYSTERY_BOUNDARY','TIMER_COUNTDOWN','MULTIDROP')),
    block_win_until_funded  BOOLEAN NOT NULL DEFAULT FALSE,
    auto_pay_win            BOOLEAN NOT NULL DEFAULT TRUE
    );
...
CREATE TABLE IF NOT EXISTS nj_drop_prize_table_item (
    ...
    prize_type       TEXT NOT NULL CHECK (prize_type IN ('JACKPOT','FIXED_CASH','NON_CASH')),
...
('DAILY','FIRST_DAY_WEEK','LAST_DAY_WEEK','FIRST_DAY_MONTH','LAST_DAY_MONTH')),
...
CREATE TABLE IF NOT EXISTS nj_jackpot_win (
    ...
    win_confirmed_status TEXT NOT NULL CHECK (win_confirmed_status IN ('PAID','ERROR','RETRIED','FAILED_TO_SEND')),
...
SELECT COALESCE(SUM(level_contrib_pct),0) INTO s
FROM nj_level
WHERE group_id = gid;
...
AFTER INSERT OR UPDATE OR DELETE ON nj_level
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW EXECUTE FUNCTION nj_trg_check_level_contrib_sum();
